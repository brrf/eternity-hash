...
class AccountInformation extends React.Component {
    ...
    fetch("http://localhost:5000/checkout/", {
      ...
      .then(resObject => {
        if (resObject.error) {
          ...
          this.props.dispatch(
            setCheckoutStep({ currentStep: 2, completed: true })
          );
        }
      });
  };
  ...
  loggedIn = () => {
    ...
    this.props.dispatch(
      setCheckoutStep({ currentStep: this.props.index + 1, completed: true })
    );
          ...
          <React.Fragment>
            {this.state.accountOwner && !this.props.authedUser ? (
              <React.Fragment>
                ...
                <p
                  onClick={this.toggleAccountOwner}
                  className="checkout-step-account-toggle"
                >
                  No account? Checkout with email only!
                </p>
              </React.Fragment>
            ) : (
              <React.Fragment>
