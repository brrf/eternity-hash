      ...
      <div>
        ...
        <div className="cart-component-container">
          <div className={"cart-container cart-large"}>
            {this.props.cart.cart.length > 0 ? (
              this.props.cart.cart.map((item, index) => {
                if (item === null || !item.piece || !item.piece.thumbnails)
                  return null;
          ...
          <div className={"cart-container checkout-details-small"}>
            <p className="checkout-details-label">Subtotal:</p>
            <span className="checkout-price-value">${subtotal}</span>
            <br />
            <p className="checkout-details-label">Shipping:</p>
            <span
              onClick={
                this.state.shipping === 0 ? this.showEstimateShipping : null
              }
              className={`checkout-price-value ${
                this.state.shipping === 0 ? "checkout-estimate" : null
              }`}
            >
            ...
            <p className="checkout-details-label">Tax:</p>
            <span
              onClick={
                this.state.tax === undefined ? this.showEstimateTax : null
              }
              className={`checkout-price-value ${
                this.state.tax === undefined ? "checkout-estimate" : null
              }`}
            >
              {this.state.tax === undefined ? "Estimate tax" : `$${tax}`}
            </span>
            <br />
            {this.state.estimateTax ? (
              ...
              >
                Delivery State: <SelectUSState onChange={this.estimateTax} />
              </div>
            ) : null}
            <hr />
            <p className="checkout-details-label">Estimated Total:</p>
            <span className="checkout-price-value">
              $
              {Math.round(
                (subtotal +
                  this.state.shipping +
                  (this.state.tax === undefined ? 0 : tax)) *
                  100
              ) / 100}
            </span>
            <br />
            <button
              ...
              disabled={!this.props.cart.cart.length}
