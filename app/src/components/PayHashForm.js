...
class PayHashForm extends React.Component {
  ...
  async submit(ev) {
    ...
    if (this.state.tax === null) {
      this.setState({
        errors: [...this.state.errors, 'Please select billing state for tax purposes']
      })
    }
    ...
    return (
      <React.Fragment>
        ...
        <div className='pay-hash-form-container' >
          <div>
            <p className='hash-order-summary' style={{fontSize: '22px'}}>Order Summary</p>
            <div className='order-summary-container'>
              <p className='order-summary-item'>Date: </p><span className='order-summary-item-value'>{date}</span><br/>
              <p className='order-summary-item'>Message: </p><span className='order-summary-item-value'>{hashObject.message}</span><br/>   
            </div>
            <hr/>
            <div className='order-summary-container'>
              <p className='order-summary-item'>Subtotal: </p><span className='order-summary-item-value'>$19.99</span><br/>
              <p className='order-summary-item'>Tax: </p><span className='order-summary-item-value'>{this.state.tax === null
          ...
          <form onSubmit={this.submit}>
            ...
            <button className='submit-button'>{total ? `Charge card $${total}` : 'Charge Card'}</button>
          </form>
        </div>
      </React.Fragment>
    )
